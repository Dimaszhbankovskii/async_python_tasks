...
Задача — проанализировать данные по погодным условиям, взяв их от API Яндекс Погоды.
...
Список городов находится в переменной `CITIES` в файле [utils.py](srcs/utils.py). Для взаимодействия с API используется готовый класс `YandexWeatherAPI` в модуле `api_client.py`. Пример работы с классом `YandexWeatherAPI` описан в <a href="#apiusingexample">примере</a>. Пример ответа от API для анализа находится в [файле](examples/response.json).

</details>

**2. Вычислить среднюю температуру и проанализировать информацию об осадках за указанный период для всех городов.**
...
**3. Объединить полученные данные и сохранить результат в текстовом файле.**
...
1. Использовать для решения как процессы, так и потоки.
2. Использовать для решения как очередь, так и пул задач.
3. Описать этапы решения в виде отдельных классов в модуле [tasks.py](srcs/tasks.py):
...
4. Использовать концепции ООП.
5. Предусмотреть обработку исключительных ситуаций.
6. Логировать результаты действий.
7. Покрыть написанный код тестами.
8. Использовать аннотацию типов.
9. Привести стиль кода в соответствие pep8, flake8, mypy.
